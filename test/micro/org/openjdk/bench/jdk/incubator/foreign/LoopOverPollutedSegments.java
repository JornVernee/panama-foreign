...
public class LoopOverPollutedSegments {
    ...
    static final VarHandle intHandleHeap =
     MemoryHandles.filterCoordinates(
            MemoryLayout.ofSequence(JAVA_INT).varHandle(int.class, MemoryLayout.PathElement.sequenceElement())
     , 0, MethodHandles.profileReferenceType(MemorySegment.class))
    ;
    ...
    @Benchmark
//    @Fork(jvmArgsAppend = { "--add-modules=jdk.incubator.foreign", "-XX:+UnlockDiagnosticVMOptions", "-XX:+LogCompilation" })
    ...
    @Fork(jvmArgsAppend = { "--add-modules=jdk.incubator.foreign",
            "-Xlog:inlining*=trace:inlining-%p-heap_segment_bytes_VH.txt"
